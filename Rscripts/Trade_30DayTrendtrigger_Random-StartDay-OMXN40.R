...
TestCycles<-10
StartDays<-sample(31:600,TestCycles) #StartDay 1 (and also ->30) is not allowed it breaks range recalculation in plotting via negative subscripting ie [-(1:1)]
...
#Sell
if(Stock==1&&(mean(Trend[(Day-30):Day])<(-1)||OMXN40Rates[Day]>1.04))
{
  Stock<-0
...
#Buy
if(Stock==0&&(mean(Trend[(Day-30):Day])>1||OMXN40Rates[Day]<0.96))
{
  Stock<-1
  Value<-Value*(1-Courtage)
  Values[ValueDay]<-Value
